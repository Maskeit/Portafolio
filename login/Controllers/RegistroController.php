...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if(empty($nombre_usuario) or empty($email) or empty($edad) or empty($passwd) or empty($passwd2)){
    ...
    if (!empty($errores)) {
        ...
        $conexionDB = new DB();
        $usuario = new Usuario($nombre_usuario, $email, $edad, $passwd);
        if($usuario->registrar($conexionDB, $nombre_usuario, $email, $edad, $passwd)){
