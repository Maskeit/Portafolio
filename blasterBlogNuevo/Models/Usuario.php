...
class Usuario{
    ...
    public function __construct(...$args) {
        $this->name = isset($args[1]) ? $args[1] : null;
        $this->user = $args[0];
    ...
    function authenticate($user){
        ...
        if($conexion == null){
            ...
            $stmt = mysqli_prepare($conexion, "SELECT * FROM username WHERE username = ? LIMIT 1");
    ...
    function registrar(DB $db,$name, $usuario, $email, $passwd) {
        ...
        $sql = 'INSERT INTO username (name, username, email, passwd) VALUES (?, ?, ?, ?)';
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $nombre_temp, $usuario_temp, $email_temp, $passwd_temp);
        $stmt->execute();
    ...
    function logear(DB $db, $usuario, $passwd){
        ...
        $sql = ('SELECT * FROM username WHERE (username = ? OR email = ?) AND passwd = ?');
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sss', $usuario, $email, $passwd);
        $stmt->execute();
        $result = $stmt->get_result();
        $fila = $result->fetch_assoc();
        if($fila !== null){
            $userFound = $fila['username'];
            $correo = $fila['email'];        
